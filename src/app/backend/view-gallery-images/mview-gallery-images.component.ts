...
@Component({
	selector: 'app-mview-gallery-images',
	templateUrl: './mview-gallery-images.component.html',
	styleUrls: ['./mview-gallery-images.component.css']
})
export class viewGalleryImagesComponent implements OnInit {
	...
	trashMe(v){				
		if(confirm("Are you sure you want to delete this?")){
			...
			this.httpClient.delete<any>(this.ip+'v1/gallery/'+v, {headers:reqHeader}).subscribe((v)=>{        
				this.getImages();
			}) 
		}
	}
	...
	getImages(){		
		...
		this.httpClient.get<{status:boolean, result: any}>(this.ip+'v1/gallery/1').subscribe(item => {											
			this.imgArr = item.result.galleries;
			console.log(this.ip+'v1/gallery/1'); 		
			console.log(item)
		})
	}
